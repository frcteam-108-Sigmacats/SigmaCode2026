...
public final class LEDModes {
  public static Function<
          Integer, Function<Integer, Function<AddressableLEDBuffer, AddressableLEDBuffer>>>
      ...
      rainbow =
          (start) -> {
            return (length) -> {
              return (buffer) -> {
                int firstPixelHue =
                    (int) ((System.currentTimeMillis() / 1000.0 * rainbowSpeed) % 180);
                for (int i = start; i < (start + length); i++) {
          ...
          (start) -> {
            return (length) -> {
              return (buffer) -> {
                ...
                for (int i = start; i < (start + length); i++) {
                  final int hue =
                      Helpers.clamp((firstPixelHue + (i * 180 / length)) % 180, 51, 255);
          ...
          (start) -> {
            return (length) -> {
              return (buffer) -> {
                int r =
                    (int)
                            (Math.pow(Math.sin(System.currentTimeMillis() / 1000.0), 10)
                                * breatheSpeed)
                        + 50;
                for (int i = start; i < (start + length); i++) {
