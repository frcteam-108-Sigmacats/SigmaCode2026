...
public class SpinDexerIOSim implements SpinDexerIO{
    ...
    private LinearSystem<N2, N1, N2> spinDexerPhysics = LinearSystemId.createDCMotorSystem(DCMotor.getNEO(1), 1.02, 1);//Have to grab the MOI and Gear ratio later
    private LinearSystem<N2, N1, N2> spinDexerKickerPhysics = LinearSystemId.createDCMotorSystem(DCMotor.getNEO(1), 1.02, 1);//Have to grab the MOI and Gear ratio later
    ...
    public SpinDexerIOSim(){
        ...
        spinDexer = new DCMotorSim(spinDexerPhysics, DCMotor.getNEO(1).withReduction(1), 0.0);//Have to grab the Gear ratio later
        spinDexerKicker = new DCMotorSim(spinDexerKickerPhysics, DCMotor.getNEO(1).withReduction(1), 0.0);//Have to grab the Gear ratio later
    ...
    public void setSpinDexerSpeed(double speed){
        spinDexerMotorVoltage = 12 * speed;//Converts the percentage of speed to the amount of volts the motor would receive to accomplish speed
        spinDexer.setInputVoltage(spinDexerMotorVoltage);//Sets the voltage we want the motor to run at
    }

    @Override
    public void setSpinDexerKickerSpeed(double speed){
        spinDexerKickerMotorVoltage = 12 * speed;//Converts the percentage of speed to the amount of volts the motor would receive to accomplish speed
        spinDexerKicker.setInputVoltage(spinDexerKickerMotorVoltage);//Sets the voltage we want the motor to run at
    }
}
