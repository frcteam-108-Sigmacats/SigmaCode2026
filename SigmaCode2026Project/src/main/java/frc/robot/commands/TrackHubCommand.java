...
public class TrackHubCommand extends Command {
  ...
  /**
   * Fraction of robot velocity used for motion compensation.
   * A value of 1.0 means full one-step prediction; tune lower if over-correcting.
   */
  private static final double MOTION_COMP_FACTOR = 0.5;

  /**
   * Shooter wheel diameter in metres – used to convert RPM setpoints from the
   * lookup table into surface speed (m/s) for {@link Turret#setShooterSpeed}.
   */
  ...
  public void execute() {
    ...
    double rpmRaw      = interpolate(rawDist, ShooterConstants.ShooterStates.shooterRPMMap);
    double hoodDegRaw  = interpolate(rawDist, ShooterConstants.ShooterStates.shooterHoodAngleMap);
    ...
    Translation2d compensatedHub = new Translation2d(
    ...
    Rotation2d fieldAngle = Rotation2d.fromRadians(
        Math.atan2(aimVec.getY(), aimVec.getX()));
   ...
   *
   * <p>If {@code key} is below the lowest key the first value is returned;
   * above the highest key the last value is returned.
   *
